...
from generator import RGB_IR_Converter

TEST_MODEL = "trainedModels\\trainedModel_200.pt"
...
def compiler(img,label,op,idx=0):
    ...
    # l1norm = np.linalg.norm(orig - recons, ord=1)
    # l["loss"] = l1norm
    return l
...
def dumpBoth(op, true, key="dump"):
    op = op[0][0].cpu().detach()
    op = op.numpy()
    op = op * 255
    op = op.astype(int)
    cv2.imwrite("output\{}_res.png".format(key), op)

    true = true[0][0].cpu().detach()
    true = true.numpy()
    true = true * 255
    true = true.astype(int)
    cv2.imwrite("output\{}_true.png".format(key), true)

if __name__ == "__main__":
    storeList = []
    testContainer = RGB_NIR_Dataset("dataset\\test_rgb", "dataset\\test_nir")
    ...
    model = RGB_IR_Converter(config, img_size=allArgs.img_size, num_classes=allArgs.num_classes).cuda()
    ...
    n = len(storeList)
    # for i in range(6):
    #     plotter(storeList[i]["img"], storeList[i]["label"], storeList[i]["op"], bNum=i + 1, keyName="random")
